<template>
  <div>
    ...
    <h2 v-else class='empty'>Список новостей пуст!</h2>
  </div>
</template>
...
export default {
  ...
  async asyncData({ $api }) {
    return { news: await $api.general.get('news') }
  },
  data() {
    return {
      news: [],
      ...
      return !this.search.length ?
        ...
        this.news.filter(el => {
          return (
            ...
            el.game.game?.toString().toLowerCase().includes(this.search.toLowerCase()) ||
            el.created?.toLowerCase().includes(this.search.toLowerCase())
          )
        })
    },
    filteredResult() {
      return this.gameFilter === 'All' ? this.results : this.results.filter(el => el.game.gameId === this.gameFilter)
    }
  },
  methods: {
