<template>
  <div class='admin-create-wrapper'>
    <div class='admin-create'>
      ...
      <AdminCreateTitle :title="id ? 'Edit League' : 'New League' " />
      ...
      <AccordeonWithSlot title='Game info'>
        ...
        <AdminUploadImage
          ...
          title='League background min 2000 x 3000'
      ...
      <AccordeonWithSlot title='Player list'>
        ...
        <BaseCheckbox v-model='isHiddenOnWebsite' on-admin text='Don’t show league on website' class='mb22' />
      </AccordeonWithSlot>
      ...
      <AccordeonWithSlot title='Tournaments'>
        <LinkWithText text='Connect tournament with league' button-text='Choose Tournaments' class='mb22' @clickBtn='openTournamentsModal' />
      </AccordeonWithSlot>
      ...
      <div class='control-buttons'>
        <div v-if='id' class='control-buttons__button' @click='removeLeague'>Delete Tournament</div>
      ...
      <div v-show='isLeaguesModalOpen' ref='leagues-modal' class='modal'>
        <div class='modal__content'>
          ...
          <div v-if='!leaguesList.length' class='modal__item'>Список турниров пуст!</div>
          ...
          >
            ...
            <div class='modal__buttons'>
              <div v-if='!ownTournaments.find(el => el.id === item.id)' class='modal__remove-button modal__remove-button_join' @click='connectTournaments(item)'>Join</div>
              <div v-else class='modal__remove-button' @click='disconnectTournaments(item)'>Leave</div>
            </div>
          </div>
...
export default {
  ...
  async asyncData({ $api, params, error }) {
    ...
    const league = await $api.general.getOne('leagues', params.id)

    if (!league && params.id !== 'add') error({ statusCode: 404 })

    const gamesData = await $api.game.getGames(10)

    const games = gamesData[0].map(el => ({ name: el.name, id: el.id }))

    return {  gamesList: games, ...league }
  },
  data() {
    return {
      ...
      radioGameType: [
        { text: 'Linked Tournaments', value: 'linked' },
        { text: 'Ladder System // not available in this MVP', value: 'ladder' }
      ],
      prizeTypes: [
  ...
  methods: {
    connectTournaments(tournament) {
      this.ownTournaments.push(tournament)
    },
    disconnectTournaments(tournament) {
      this.ownTournaments = this.ownTournaments.filter(el => el.id !== tournament.id)
    },
    async removeLeague() {
      try {
        ...
        await Promise.all([
          this.game.mainBannerInitial ? await this.$api.general.removeImage('leagues', this.id, this.game.mainBannerInitial) : Promise.resolve,
          this.game.tournamentBgInitial ? await this.$api.general.removeImage('leagues', this.id, this.game.tournamentBgInitial) : Promise.resolve,

          ...this.sponsors.map(async (el, index) => {
            if (el.initialImage) {
              await this.$api.general.removeImage('leagues', this.id, el.initialImage, `sponsor-${index}`)
              await this.$api.general.removeImage('leagues', this.id, el.initialImage, `sponsor-${index}`)
            }
          })
        ])

        await this.$api.general.remove('leagues', this.id)
    ...
    async finishTournament() {
      try {
        ...
        await this.$api.general.update('leagues', this.id, {
          ongoing: false,
        })
    ...
    async openTournamentsModal() {
      try {
        this.$nuxt.$loading.start()

        if (!this.leaguesList.length) {
          const tournaments = await this.$api.general.get('tournaments')

          this.leaguesList = tournaments.map(el => ({ title: el.basicInfo.title, id: el.id }))
        }
    ...
    async saveTournament() {
      try {
        ...
        await Promise.all([
          this.game.mainBannerInitial ? await this.$api.general.removeImage('leagues', id, this.game.mainBannerInitial, 'mainBanner') : Promise.resolve,
          this.game.tournamentBgInitial ? await this.$api.general.removeImage('leagues', id, this.game.tournamentBgInitial, 'tournamentBg') : Promise.resolve,

          this.game.mainBanner = this.game.mainBannerFile ? await this.$api.general.uploadImage(this.mainBannerFile, 'leagues', id, this.mainBannerFileName, 'mainBanner') : this.game.mainBanner,
          this.game.mainBannerFile = null,
          this.game.tournamentBg = this.game.tournamentBgFile ? await this.$api.general.uploadImage(this.tournamentBgFile, 'leagues', id, this.tournamentBgFileName, 'tournamentBg') : this.game.tournamentBg,
          this.game.tournamentBgFile = null,

          ...this.sponsors.map(async (el, index) => {
            if (el.imageFile && el.initialImage) {
              await this.$api.general.removeImage('leagues', id, el.initialImage, `sponsor-${index}`)
            }

            if (el.imageFile)
              this.sponsors[index].image = await this.$api.general.uploadImage(el.imageFile, 'leagues', id, el.imageFileName, `sponsor-${index}`)
        ...
        const data = {
          ...
          rules: this.rules,
          ongoing: true,
          ownTournaments: this.ownTournaments
        }

        this.id ? await this.$api.general.update('leagues', id, data) : await this.$api.general.set('leagues', id, data)
